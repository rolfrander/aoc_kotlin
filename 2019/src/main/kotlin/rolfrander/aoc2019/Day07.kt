...
suspend fun runWithInput(vm: Intcode, phaseSetting: Int, input: Int): Int {
    vm.inputs = sequenceOf(phaseSetting, input)
    vm.run()
    return vm.outputs.first()
}

suspend fun run(program: List<Int>, input: Array<Int>): Int {
    var out = 0
    for(i in 0..4) {
...
suspend fun runWithChannels(program: List<Int>, 
                            i: Int,
                            inChan: ReceiveChannel<Int>,
                            outChan: SendChannel<Int>) {
    val vm = Intcode(program, i)
...
class Day07 @Autowired constructor(config: AocData): AocBase(config, 7, """3,15,3,16,1002,16,10,16,1,16,15,15,4,15,99,0,0""") {
    ...
    override fun part1(data: String): Any {
        return runBlocking {
            ...
            for (perm in permutations(arrayOf(0,1,2,3,4))) {
                best = maxOf(best, run(program, perm))
            }
            best
        }
    }
